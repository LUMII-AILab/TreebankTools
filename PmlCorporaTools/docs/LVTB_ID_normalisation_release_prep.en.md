...
Lets assume that `.` folder is `TreebankTools/PmlCorporaTools` and that data to be checked is flattly (no subfolders!) coppied in folder `./data/original`. Results wil be given in the `./data/normalizedIds`.
